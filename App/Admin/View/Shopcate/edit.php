...
<html>
	...
	<body>
		...
		<form action="" method="post">
			...
			<div class="form-group">
				...
				<select name="shop_type_tid">
					...
					{{foreach from="$typeData" value="$v"}}
						<option value="{{$v['tid']}}" {{if value="$v['tid'] eq $oldData['shop_type_tid']"}}selected{{endif}}>{{$v['tname']}}</option>
					{{endforeach}}
				</select>
			</div>
			<div class="form-group">
			...
			<input type="hidden" name="pid" id="pid" value="{{$oldData['pid']}}" />
			<input type="hidden" name="cid" id="cid" value="{{$oldData['cid']}}" />
