							...
							<ul>
								...
								{{foreach from="$brand" key="$k" value="$v"}}
									{{foreach from="$v" value="$value"}}
										<li data_initial="{{$k}}">
											<a href="{{U('index',array('seach'=>$_GET['seach'],'param'=>Q('get.param'),'bid'=>$value['bid']))}}" {{if value="$bid eq $value['bid']"}}style="color:red;"{{endif}}>{{$value['bname']}}</a>
										</li>
									{{endforeach}}
								{{endforeach}}
							</ul>
							...
							<div class="pp">
								<ul>
									...
									{{foreach from="$v['value']" value="$value"}}
										...
										?>
										<li>
											<a href="{{U('index',['seach'=>$_GET['seach'],'param'=>implode('_',$temp),'bid'=>isset($_GET['bid'])?$_GET['bid']:0])}}" {{if value="$value['gtid'] eq $param[$k]"}}style="color:red;font-weight:700;"{{endif}}>{{$value['gtvalue']}}</a>
										</li>
									{{endforeach}}
								</ul>
							</div>
