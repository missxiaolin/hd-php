...
<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
	<head>
		...
		<title>商品列表</title>
		<!--分页-->
		<link rel="stylesheet" type="text/css" href="./Public/index/css/page/zzsc.css" />
		<link rel="stylesheet" type="text/css" href="css/page/simplePagination.css" />
		<!--*****************************************************-->
...
//				删除指定不要的商品
		...
		<?php if(isset($finalAttr)){?>
                
			<div id="crumbs_nav">
				全部结果<i>></i><b>"<?php echo Q('get.seach')?>"</b>
			</div>
		<?php }else{?>
			<div id="crumbs_nav">
				<div class="ns_wrap">
					...
					<div class="middle">
						...
						<ul class="sx">
							...
							<?php foreach ($brand as $k=>$v){?>
								<li data_initial="<?php echo $k?>"><?php echo $k?></li>
							<?php }?>
						</ul>
						<div class="clr"></div>
						<div class="pp">
							<ul>
								...
								<?php foreach ($brand as $k=>$v){?>
									<?php foreach ($v as $value){?>
										<li data_initial="<?php echo $k?>">
											<a href="<?php echo U('index',array('seach'=>$_GET['seach'],'param'=>Q('get.param'),'bid'=>$value['bid']))?>" <?php if($bid==$value['bid']){?>
                style="color:red;"
               <?php }?>><?php echo $value['bname']?></a>
										</li>
									<?php }?>
								<?php }?>
							</ul>
							...
							<div class="pp">
								<ul>
									...
									<?php foreach ($v['value'] as $value){?>
										...
										?>
										<li>
											<a href="<?php echo U('index',['seach'=>$_GET['seach'],'param'=>implode('_',$temp),'bid'=>isset($_GET['bid'])?$_GET['bid']:0])?>" <?php if($value['gtid']==$param[$k]){?>
                        	...
                        	<?php foreach ($spreadData as $v){?>
	                        	<li>
									...
									<div class="p_price">
			                            <strong>
			                                <em>¥</em>
			                                <span><?php echo $v['shopprice']?>.00</span>
			                            </strong>
			                        </div>
									<div class="p_name">
									...
									<div class="p_comm">
										已有<span><?php echo $v['count']?></span>人评价 
									</div>
	                        	</li>
                        	<?php }?>
						...
						<?php if(isset($shopData)){?>
							...
							<?php foreach ($shopData as $v){?>
								<li>
									<div class="gl_i_wrap">
										<!--图像-->
										<a href="shop_<?php echo $v['gid']?>.html">
											<div class="p_img">
										...
										<div class="p_price">
											<span>￥</span><i><?php echo $v['shopprice']?>.00</i>
										</div>
										<div class="p_icons">
											<a href="shop_<?php echo $v['gid']?>.html"><?php echo $v['gname']?></a>
										</div>
										<div class="p_commit">
											<strong>已有<a href=""><?php echo $v['count']?>+</a>评价</strong>
										</div>
									</div>
								</li>
							<?php }?>
